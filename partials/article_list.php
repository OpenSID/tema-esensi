...
<div class="max-w-full w-full bg-white shadow rounded-lg p-5 border">
  <figure class="h-auto lg:h-32 w-1/3 float-left pr-3 pb-3 <?php $image === gambar_desa($desa['logo']) and print('lg:h-auto') ?>">
    <img src="<?= $image ?>" alt="<?= $post['judul'] ?>" class="<?php $image !== gambar_desa($desa['logo']) and print('h-auto lg:h-32 w-full object-cover object-center') ?> max-w-full mx-auto">
  </figure>
  <a href="<?= $url ?>" class="text-h5 hover:text-primary-100 mb-3"><?= $post['judul'] ?></a>
  <p class="line-clamp-4 mb-3"><?= $abstract ?></p>
